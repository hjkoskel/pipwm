...
func main() {
	...
	if (0 <= pLo) && (0 <= pHi) {
		setPwm0Generator(pHi, pLo, pVerbose)
		return
	}
	...
	if (0 <= pPercent) && (pPercent <= 100) { //Percent is defined try calculating missing info
		...
		if 0 <= pTr {             //period and percent defined
			setPwm0Generator(pTr*ratio, pTr*(1.0-ratio), pVerbose)
			return
		}
		if (pLo < 0) && (!(pHi < 0)) { //Hi time is known, but not lo... so calc period
			...
			setPwm0Generator(pHi, pHi*(1/ratio-1), pVerbose)
			return
		}
		if (pHi < 0) && (!(pLo < 0)) { //Lo time is known, but not hi... so calc period
			...
			setPwm0Generator(ratio*pLo/(1-ratio), pLo, pVerbose)
			return
		}
	}
	...
	if 0 < pTr {
		if 0 <= pHi {
			...
			setPwm0Generator(pHi, pTr-pHi, pVerbose)
			return
		}
		if 0 <= pLo {
			if pTr < pLo {
			...
			setPwm0Generator(pTr-pLo, pLo, pVerbose)
			return
		}
	}
